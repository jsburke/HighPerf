...
void *PrintHello(void *threadarg)
{
  ...
  case 7:
    ...
    break;
  case 8:
    printf("\nIt's me, thread #%ld! I'm waiting for 6 and 7 ...", taskid);
    pthread_mutex_lock(&mutexA[7]);
    pthread_mutex_unlock(&mutexA[7]); // putting 7 first lets them all unlock
    pthread_mutex_lock(&mutexA[6]);   // 8 can block 7 on 6 otherwise and it gets stuck
    pthread_mutex_unlock(&mutexA[6]);
  break;
  default:
