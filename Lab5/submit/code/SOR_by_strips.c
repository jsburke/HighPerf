...
void* thr_SOR_wrkr1(void* t_arg);
void thr_SOR_1(vec_ptr v, long int itermax);
void* thr_SOR_wrkr2(void* t_arg);
void thr_SOR_2(vec_ptr v, long int itermax);
...
#define TIMETYPE CLOCK_REALTIME
// #define TIMETYPE CLOCK_PROCESS_CPUTIME_ID
...
double measure_cps()
{
  ...
  for(j=0; j<2; j++) {
    clock_gettime(TIMETYPE, &cal_start);
    MCPS_RDTSC(tsc_start);
    ilim = 50*1000*1000;
    for (i=0; i<ilim; i++)
      z = z * z + chaosC;
    clock_gettime(TIMETYPE, &cal_end);
    MCPS_RDTSC(tsc_end);
  }
...
void* thr_SOR_wrkr1(void* t_arg)  // split the SOR on red-black
{
  //  thread struct
...
void thr_SOR_1(vec_ptr v, long int itermax)
{
  ...
  for (t=0; t<NTHREADS; t++) {
    ...
    rv = pthread_create(&threads[t], NULL, thr_SOR_wrkr1, (void*) &thr_args[t]);
  }
  for (t=0; t<NTHREADS;t++) {
    rv = pthread_join(threads[t], NULL);
  }
  pthread_barrier_destroy(&sor_barrier);
}



void* thr_SOR_wrkr2(void* t_arg)  // split the SOR on red-black
{
  //  thread struct
  sor_data * thread_arg = (sor_data *) t_arg;
  int t_id = thread_arg -> t_id;
...
void thr_SOR_2(vec_ptr v, long int itermax)
{
  ...
  for (t=0; t<NTHREADS; t++) {
    ...
    rv = pthread_create(&threads[t], NULL, thr_SOR_wrkr2, (void*) &thr_args[t]);
  }
  for (t=0; t<NTHREADS;t++) {
    rv = pthread_join(threads[t], NULL);
  }
